...
def blood_transfusion_svm(data: pd.DataFrame, targets: pd.Series) -> None:
    ...
    validation_curve(svm, data, targets)
...
def blood_transfusion_dummy_classifier(
    ...
    scores = model.kfold_cross_validate(data, targets, 10, scoring="balanced_accuracy")
    model.print_kfold_cross_validation_accuracy(scores)
...
def blood_transfusion_knearest_classifier(
    ...
    scores = model.kfold_cross_validate(
        data, targets, nb_fold, scoring="balanced_accuracy", return_train_score=True
    )
    if print_score:
        model.print_kfold_cross_validation_accuracy(scores)
...
def tune_knearest_classifier(data: pd.DataFrame, targets: pd.Series):
    ...
    curve = model.compute_validation_curve(data,
                                           targets,
