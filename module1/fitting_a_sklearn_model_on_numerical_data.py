...
def first_model_with_scikit_learn(x_data: pd.DataFrame,
    ...
    # check_data(x_data, y_data)

    # 2. Select some feature (just for an example of a simple model)
    data = dh.get_subset(x_data, columns=["age", "capital-gain", "capital-loss", "hours-per-week"])

    # 3. Randomnly split data between train and test set
    train_test_split = dh.manual_train_test_split(data, y_data)

    # 4. Build model
    k_neighbors = KNeighborsClassifierModel.build()
    k_neighbors.start(**train_test_split)
    k_neighbors = KNeighborsClassifierModel.build(n_neighbors=50)
    k_neighbors.start(**train_test_split)
...
def working_with_numerical_data(x_data: pd.DataFrame,
    ...
    logistic_regression = LogisticRegressionModel.build()
    logistic_regression.start(**train_test_split)

    # 4. Build dummy classifier model
    high_income = DummyClassifierModel.build(constant=">50K")
    high_income.start(**train_test_split) # catastrophic (maybe highlight a class inbalance)

    low_income = DummyClassifierModel.build(constant="<=50K")
    low_income.start(**train_test_split) # ok


def preprocessing_for_numerical_features(x_data: pd.DataFrame,
                                         y_data: pd.Series) -> None:
    ...
    if not use_pipeline:
        ...
        x_train_scaled = dh.standard_scaler(train_test_split["x_train"])

        # 3bis. Check scaler effect on data
        print(x_train_scaled.describe())
        scaler_jointplot(train_test_split["x_train"],
                         x_train_scaled,
                         x_axis="age",
...
def model_evaluation_using_cross_validation(x_data: pd.DataFrame,
                                            y_data: pd.Series) -> None:
...
def run_analysis():
    ...
    data, targets = _load_numerical_data(adult_census["x_data"]), adult_census["y_data"]
