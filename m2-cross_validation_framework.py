...
"""
def load_californiad_dataset() -> tuple[pd.DataFrame, pd.Series]:
...
def simple_test_continuous_target_prediction(data: pd.DataFrame, targets: pd.Series):
    ...
    regressor.train(regressor.model, data, targets)
...
def test_continuous_target_prediction(data: pd.DataFrame, targets: pd.Series):
    ...
    # 4. Performe a shuffle-split cross validation.
    scores = regressor.shuffle_split_cross_validate(data, targets, 40, "neg_mean_absolute_error")
    regressor.print_shuffle_split_cross_validation_accuracy(scores)

    scores = regressor.shuffle_split_cross_validate(data, targets, 40, "neg_mean_absolute_percentage_error")
    regressor.print_shuffle_split_cross_validation_accuracy(scores)


if __name__ == "__main__":
    # Load data
    data, targets = load_californiad_dataset()

    # simple_test_continuous_target_prediction(data, targets)
    test_continuous_target_prediction(data, targets)